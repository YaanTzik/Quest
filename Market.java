...
public class Market{
    ...
    //Fire Spells
    private Spell FlameTornado = new Spell("Flame Tornado",700,4,850,300,"Fire");
    private Spell BreathOfFire = new Spell("Breath of Fire", 350,1,450,100,"Fire");
    private Spell HeatWave = new Spell("Heat Wave", 450,2,600,150,"Fire");
    private Spell LavaCommet = new Spell("Lava Commet", 800,7,1000,550,"Fire");
    
    //Ice Spells
    private Spell SnowCannon = new Spell("Snow Cannon",500,2,650,250,"Ice");
    private Spell IceBlade = new Spell("Ice Blade", 250,1,450,100, "Ice");
    private Spell FrostBlizzard = new Spell("Frost Blizzard", 750,5,850,350, "Ice");
    private Spell ArcticStorm = new Spell("Arctic Storm", 700,6,800,300, "Ice");

    //Lightning Spells
    private Spell LightningDagger = new Spell("Lightning Dagger",400,1,500,150,"Lightning");
    private Spell ThunderBlast= new Spell("Thunder Blast", 750, 4, 950,400,"Lightning");
    private Spell ElectricArrows = new Spell("Electric Arrows", 550,5,650,200,"Lightning");
    private Spell SparkNeedles = new Spell("Spark Needles", 500, 2,600,200,"Lightning");
        ...
        while (!close) {
            ...
            if (choice == 5) {
                ...
                if (mode.equals("buy")) {
                    ...
                    if(eq != null) h.PurchaseItem(eq, h.getBag().getWeapons());
                ...
                if ( mode.equals("buy")) {
                    ...
                    if (eq!= null)h.PurchaseItem(eq, h.getBag().getArmors());
                } 
                else if(mode.equals("sell")){
                ...
                if (mode.equals("buy")){
                    ...
                    if (pot != null)h.PurchaseItem(pot, h.getBag().getPotions());
                } 
                    
                else if(mode.equals("sell")){
                    pot = h.getBag().openSubbag(h.getBag().getPotions()); 
                    if(pot != null)h.sellItem(pot);
                }
                ...
                if (mode.equals("buy")) {
                    ...
                    if (sp!= null) h.PurchaseItem(sp, h.getBag().getSpells());
                } 
            else if(mode.equals("sell")){
                    sp = h.getBag().openSubbag(h.getBag().getSpells());
                    if (sp!= null)h.sellItem(sp);
                }
    ...
    public <T extends Item>  T ListForPurchase(ArrayList<T> window){
        ...
        else return window.get(choice-1);
    }
}